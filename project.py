...
def visualize_data(data):
    ...
    print("\n--- Generating Plots for Data Visualizations ---")
    output_dir = 'plots'
    ...
    sns.histplot(data['Salary'], kde=True, bins=30, color='lightpink')
    plt.title('Distribution of Employee Salary', fontsize=16)
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x='Years of Experience', y='Salary', data=data, alpha=0.6, color='green')
    plt.title('Scatter Plot of Salary v/s Years of Experience', fontsize=16)
    ...
    plt.title('Box Plot of Salary Distribution by Job Title', fontsize=16)
    plt.xlabel('Salary (INR)')
    ...
    edu_salary = data.groupby('Education Level')['Salary'].mean().reset_index()
    plt.pie(edu_salary['Salary'], labels=edu_salary['Education Level'], autopct='%1.1f%%', startangle=140, colors=sns.color_palette('pastel'))
    plt.title('Salary Distribution by Education Level', fontsize=16)
    plt.savefig(os.path.join(output_dir, 'salary_by_education_level.png'))
    plt.show()
...
if __name__ == "__main__":
    ...
    print(f"Data generation completed and Saved to '{full_path}'")
